...
updatecopy(rccopy){
  ...
  exportPANData(){
    this.truckPanService.exportData().subscribe(
      (res) => {
        if(res.success){
          this.windowRef.nativeWindow.open(AppConfig.API_ENDPOINT + "/pan/downloadData?key=" + res.data, '_blank');
        }else{
          this.toaster.error(res.message);
        }
        this.toaster.success("PAN data exported successfully");
      },
      () => {
        this.toaster.error("Error while exporting PAN data");
      }
    )
  }
}
